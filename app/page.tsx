    ...
    <div className="relative z-10 w-full flex flex-col items-center">
      ...
      <div className="
        ...
        {[PIN_DATA.left, PIN_DATA.right].map((column, colIdx) => (
          <div key={colIdx} className="flex flex-col justify-between gap-[0.5rem] sm:gap-[0.9rem]">
            {column.map((conn) => (
              <div key={conn.id} className="relative bg-[#D1D5DB] p-1 pt-4 sm:p-[0.25rem] sm:pt-[1rem] rounded-sm border border-zinc-400">
                ...
                {conn.rows.map((row, i) => {
                  ...
                  return (
                    <div key={i} className={`flex items-center h-[1.1rem] sm:h-[1.4rem] text-[0.55rem] sm:text-[0.7rem] font-bold ${(matchP1 || matchP2) ? 'bg-yellow-300/50' : ''}`}>

                      {/* LEWA ETYKIETA */}
                      <div className="flex w-16 sm:w-24 justify-end items-center pr-1 sm:pr-2 gap-1">
                        <span className="w-8 sm:w-[2.5rem] text-right truncate" style={{ color: getHex(row.colors[0]) }}>{row.p1[1]}</span>
                        <span className="w-10 sm:w-[2.8rem] text-right" style={{ color: getHex(row.colors[0]) }}>{row.p1[0]}</span>
                      </div>

                      {/* PIN 1 i 2 */}
                      <div className={`w-4 h-4 sm:w-6 sm:h-6 shrink-0 flex items-center justify-center text-white border border-black/30 rounded-[1px] text-[0.5rem] sm:text-[0.7rem] ${matchP1 ? 'scale-125 ring-1 ring-black z-10' : ''}`} style={{ backgroundColor: getHex(row.colors[0]) }}>{row.ids[0]}</div>
                      <div className={`w-4 h-4 sm:w-6 sm:h-6 shrink-0 flex items-center justify-center text-white border border-black/30 rounded-[1px] text-[0.5rem] sm:text-[0.7rem] ${matchP2 ? 'scale-125 ring-1 ring-black z-10' : ''}`} style={{ backgroundColor: getHex(row.colors[1]) }}>{row.ids[1]}</div>
